...
export async function storeFiles({
  ...
  typeof unidad_adm === 'object' ? (unidad_adm = unidad_adm.value) : unidad_adm;
  typeof anexo === 'object' ? (anexo = anexo.value) : anexo;

  //   console.log('nombre and unidad', nombre);
  //   console.log('unidad', unidad_adm);
  //   console.log('anexo', anexo);
  const filePaths = { documentoPath: [], fotoPath: [] }; // Initialize filePaths object

  if (!(nombre && unidad_adm) && !isUpdate)
  ...
  const uploadFile = async (folder, subFolder, anexo, fileName, file) => {
    const { data: uploadDoc, error } = await supabase.storage
      .from(folder || 'audits')
  ...
  try {
    ...
    if (documento && documento.length > 0) {
      for (let doc of documento) {
        ...
        if (doc.path) {
          filePaths.documentoPath.push({
            fileName: doc.fileName,
            path: doc.path,
          });
        } else {
          const newDocumentPath = await uploadFile(
            'audits',
            unidad_adm,
            anexo,
            nombre,
    ...
    if (foto && foto.length > 0) {
      for (let pic of foto) {
        if (pic.file) {
          ...
          const newPhotoPath = await uploadFile(
            'audits',
            unidad_adm,
            anexo,
            nombre,
...
export async function deleteFiles(filePaths, folder) {
  ...
  const filesToDelete = Array.isArray(filePaths)
    ? filePaths.flatMap((filePath) => filePath.path)
    : [filePaths.path];
